...
public class DatosApiService {
    ...
    public void loadGenres() {
        ...
        try {
            ...
            if (tmdbGenres != null && !tmdbGenres.isEmpty()) {
                ...
                System.out.println("   -> Insertados/Actualizados " + localGenres.size() + " géneros en la Base de Datos.");
            } else {
                System.out.println("   -> [WARN] La API devolvió una lista vacía de géneros.");
            }
        } catch (TmdbException e) {
            throw ErrorFactory.internal("Error de comunicación con TMDB: " + e.getMessage());
        } catch (Exception e) {
            throw ErrorFactory.internal("Error inesperado cargando géneros.");
        }
    }
    ...
    public void loadMovies() {
        ...
        try {
            ...
            throw ErrorFactory.internal("Error inesperado durante la carga de películas.");
        }
    }
    ...
    private void flushBuffers(ExternalApiService externalApiService, List<Movie> movies, List<service.ExternalApiService.PersonWithCharacter> actors, List<entity.Person> directors, List<entity.Country> countries, Map<Integer, List<service.ExternalApiService.PersonWithCharacter>> mapMovieActors, Map<Integer, List<entity.Person>> mapMovieDirectors, Map<Integer, List<entity.Country>> mapMovieCountries, MovieController movieCtrl, PersonController personCtrl, CountryController countryCtrl) {
        System.out.println("      -> [BATCH] Guardando lote de " + movies.size() + " películas y relaciones...");
        ...
        for (Movie m : movies) {
            ...
            if (movieCast != null) {
                for (var actor : movieCast) {
                  ...
                  if (dbPersonId != null) {
                      batchCast.add(new Object[]{ m.getMovieId(), dbPersonId, actor.getCharacterName() });
                  }
                }
            }
            ...
            if (movieCrew != null) {
                for (var dir : movieCrew) {
                  ...
                  if (dbPersonId != null) {
                       batchCrew.add(new Object[]{ m.getMovieId(), dbPersonId, "Director" });
                  }
                }
            }
        ...
        if (!batchCast.isEmpty()) movieCtrl.updateMovieActors(batchCast);
        if (!batchCrew.isEmpty()) movieCtrl.updateMovieDirectors(batchCrew);
        if (!batchCountries.isEmpty()) countryCtrl.saveBatchRelations(batchCountries);
    ...
    public void loadActorsDirectorsAndRuntime() {
        ...
        for (Movie movie : allMovies) {
            ...
            try {
                ...
                // Silencioso para no ensuciar la consola durante cargas largas
            }
        }
    ...
    public void loadPersons() {
        ...
        System.out.println("   -> Total personas a revisar: " + allPersons.size());
        ...
        for (entity.Person personaOriginal : allPersons) {
            ...
            // Skip si ya tiene todos los datos clave
            ...
            try {
                if (contador % 100 == 0) {
                    System.out.println("   -> Revisando persona " + contador + "/" + allPersons.size());
                }
                ...
                if (bufferPersons.size() >= BATCH_SIZE) {
                 ...
                 // Falla silenciosa permitida para evitar que el script se detenga
            }
        }
